/**
 * Unit tests for NirnamBus, run once per hub kind.
 *
 * Worker, SharedWorker, MessageChannel, BroadcastChannel, Blob and
 * URL.createObjectURL are replaced by the synchronous mocks in tests/setup.ts;
 * routing goes through the real MessageHub. Delivery is synchronous, so most
 * assertions need no awaiting. The request/handle pair uses
 * Promise.resolve().then() internally, so those tests await the returned
 * Promise.
 *
 * Hub selection, fallback and worker lifetime are covered in hub-port.test.ts.
 */
...
import { createBus as realCreateBus, NirnamBus } from '../src/bus';
import { NirnamErrorCode, NirnamRequestError } from '../src/types';
import type { HubKind, NirnamBusOptions } from '../src/types';
import type { BusPort } from '../src/hub-port';
import {
  resetWorkerState,
...
const HUBS: HubKind[] = ['inline', 'dedicated', 'shared'];
...
function internals(bus: NirnamBus) {
  return bus as unknown as {
    port: BusPort & { postMessage: jest.Mock };
...
/** Deliver a message to the bus as if the hub had sent it. */
function receive(bus: NirnamBus, data: unknown) {
  internals(bus).port.onmessage?.({ data } as MessageEvent);
}

describe.each(HUBS)('hub: %s', (hub) => {
  /** createBus() pinned to this hub, with the port's postMessage spied for every kind. */
  const createBus = (options: NirnamBusOptions = {}) => {
    const bus = realCreateBus({ hub, ...options });
    const port = internals(bus).port;
    if (!jest.isMockFunction(port.postMessage)) jest.spyOn(port, 'postMessage');
    return bus;
  };
  ...
  describe('subscribe / publish', () => {
    ...
    it('removing last handler sends unsubscribe to worker', () => {
      const bus = createBus();
      const handler = jest.fn();
      const { port } = internals(bus);

      const unsub = bus.subscribe('topic', handler);
      unsub();

      expect(port.postMessage).toHaveBeenCalledWith({ type: 'unsubscribe', topic: 'topic' });
    });
    ...
    it('removing one of two handlers does not unsubscribe from worker', () => {
      ...
      const calls = port.postMessage.mock.calls.map((c: unknown[]) => (c[0] as { type: string }).type);
    ...
    it('publishes only once to worker per call', () => {
      ...
      const broadcastCalls = port.postMessage.mock.calls.filter(
  ...
  describe('close()', () => {
    it('leaves the hub: a later publish no longer reaches the closed bus', () => {
      const closed = createBus();
      const other = createBus();
      const handler = jest.fn();
      closed.subscribe('topic', handler);

      closed.close();
      other.publish('topic', 'after-close');
  ...
  describe('options', () => {
    ...
    it('broadcast worker message for topic with no local handler is silently ignored', () => {
      ...
      const bus = createBus();
          expect(() => {
        receive(bus, { type: 'broadcast', topic: 'unregistered-topic', payload: 'x' });
      }).not.toThrow();
      bus.close();
    });
    ...
    it('worker broadcast message without topic field is ignored (`if (topic)` false branch)', () => {
      const bus = createBus();
          expect(() => {
        receive(bus, { type: 'broadcast', payload: 'x' }); // no topic
      }).not.toThrow();
      bus.close();
    });
    ...
    it('worker stream-end for unknown requestId is ignored (`if (stream)` false branch)', () => {
      const bus = createBus();
          expect(() => {
        receive(bus, { type: 'stream-end', requestId: 'no-such-stream' });
      }).not.toThrow();
      bus.close();
    });
    ...
    it('workerUrl reaches the worker constructor; the inline hub creates no worker', () => {
      const bus = createBus({ workerUrl: '/static/nirnam-worker.js' });
      expect(getLastWorkerUrl()).toBe(hub === 'inline' ? null : '/static/nirnam-worker.js');
      bus.close();
    });

    it('the Blob URL is used when workerUrl is not set', () => {
      const bus = createBus();
      expect(getLastWorkerUrl()).toBe(hub === 'inline' ? null : 'blob:mock-url');
      bus.close();
    });
  ...
  describe('register / discoverAgents / onAgentChange', () => {
    ...
    it('sends watch-agents only once regardless of handler count', () => {
      ...
      const watchCalls = port.postMessage.mock.calls.filter(
